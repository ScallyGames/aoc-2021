...
public record struct Cuboid
{
    ...
    public bool IsOverlapping(Cuboid other, bool shouldLog = false)
    {
        if(shouldLog)
        {
            if(!IsAxisOverlapping(this.MinX, this.MaxX, other.MinX, other.MaxX))
            {
                Console.WriteLine("not overlapping on X");
            } else if(!IsAxisOverlapping(this.MinY, this.MaxY, other.MinY, other.MaxY))
            {
                Console.WriteLine("not overlapping on Y");
            } else if(!IsAxisOverlapping(this.MinZ, this.MaxZ, other.MinZ, other.MaxZ))
            {
                Console.WriteLine("not overlapping on Z");
            }
        }
        ...
        CalculateOverlapResults(Cuboid overlappingSegment, Cuboid other, Axis axis)
    {
        ...
        if(
            other == Cuboid.Default ||
            overlappingSegment == Cuboid.Default ||
            !IsAxisOverlapping(
        ...
        var newOverlappingSegment = cutResults.Where(x => x.Size > 0 && x.IsOverlapping(other)).SingleOrDefault(Cuboid.Default);
